...
The addon source under this directory is copied from the pinned npm package. The matching MCP server version is pinned in `.vscode/mcp.json` and `opencode.json`. The recorded SHA-512 documents the reviewed npm tarball; `npx` does not enforce this value as a local lockfile would.
